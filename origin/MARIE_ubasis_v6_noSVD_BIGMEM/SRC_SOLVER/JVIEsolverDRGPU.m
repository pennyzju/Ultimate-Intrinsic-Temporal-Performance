...
% using restarted GMRES with Deflation (DR) 
...
%
%
% SUBFUNCTIONS: fA_d2, JVIEsolverDRGPU_iter.m
%
%


function [x,flag,relres,iter,resvec] = JVIEsolverDRGPU(fG, tau, L, M, N, dx3, idx, b, restart, tol, maxit, ritz, x0)
...
idx_gpu = gpuArray(idx);
r_gpu = gpuArray(r);

% Call the gmres to perform restart iterations
[V,H,~,p,resvec_inner] = JVIEsolverDRGPU_iter(r_gpu,restart,C,tol*bnorm,fG_gpu,tau_gpu,L,M,N,dx3,idx_gpu);

% get gpu data
H = gather(H);
V = gather(V);
resvec_inner = gather(resvec_inner);
...
while(resvec(it) > tol) && (outit < maxit)
    ...
    % Call the gmres to perform restart iterations ------------------------
    [V,H,B,p,resvec_inner] = JVIEsolverDRGPU_iter(r_gpu,restart,C_gpu,tol*bnorm,fG_gpu,tau_gpu,L,M,N,dx3,idx_gpu);
