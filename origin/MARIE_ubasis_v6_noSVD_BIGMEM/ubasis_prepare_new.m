...
function ubasis_prepare_new(out_dir,epsilon_r,sigma_e,rho,r,f,Dmin,Dmax,regions_dipoles,no_dipole_region)
...
ind_check = find( epsilon_r(:)<1 | sigma_e(:)<0  | rho(:)<0, 1  );%逻辑运算符 或者检查材料属性是否正确
...
% generate dipoles
filename = sprintf('%s/dist_dipolesObject.mat',out_dir);%生成dipole距离矩阵文件名称
mask = double( epsilon_r>1 & sigma_e>0 & rho>0 );
if ~exist(filename,'file') %不存在距离矩阵
...
% shave off boundary object voxels修剪边界对象体素
mask2 = ubasis_shave_boundary_voxels(mask);

epsilon_r = matrixTranslate(epsilon_r);
sigma_e = matrixTranslate(sigma_e);
rho = matrixTranslate(rho);
...
function ubasis_comp_dist_dipoleObject(mask,r,out_dir)%生成dipole距离矩阵
