...
{  
  ...
  int N_theta = (int) mxGetScalar(prhs[5]);
  int N_psi   = (int) mxGetScalar(prhs[6]);
...
//   mexPrintf("z_psi =  (%f,%f,%f) \n", z_psi[7],z_psi[8],z_psi[9]);
  ...
  complex<double> I_DE = DIRECT_WS_EA_const( r1, r2, r3, r4, ko, N_theta, N_psi, z_theta, w_theta, z_psi, w_psi );
  I_DEr[0] = real(I_DE);
  I_DEi[0] = imag(I_DE);
}
