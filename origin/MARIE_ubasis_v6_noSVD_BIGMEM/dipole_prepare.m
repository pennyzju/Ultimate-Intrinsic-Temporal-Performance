...
function dipole_prepare(out_dir,epsilon_r0,sigma_e0,rho0,epsilon_r,sigma_e,rho,r,f,Dmin,Dmax,regions_dipoles,no_dipole_region)
...
mask0 = double( epsilon_r>1 & sigma_e>0 & rho>0 );
figure;
subplot(2,2,1); imagesc(squeeze(mask0(ceil(end/2),:,:))); axis image off; title('mask');
...
if ~exist(filename,'file')
    ubasis_comp_dist_dipoleObject(mask0,r,out_dir);
else
    load(filename);
    if size(mask_dip) ~= size(mask0)
        ubasis_comp_dist_dipoleObject(mask0,r,out_dir);
    end
end
...
ind_check2 = find( epsilon_r(:)<1 | sigma_e(:)<0  | rho(:)<0  );
if ~isempty(ind_check2)
    error('Incorrect material maps!');
end
...
eval( sprintf('save %s f r epsilon_r sigma_e rho idxS idxI idxN mask mask2 dx dy dz pvol',filename) );%idxN mask mask2 dy dz 都用不到
