function snr_process_sos_mag_all_new(FolderPath, flag, angle_deg)
    % folderpath: 输出根目录
    ...
    for i = 1:length(subfolders)
        folder_name = fullfile(FolderPath, 'sos_mag_all_new',subfolders{i});
    ...
    for i = 1:length(angle_matrix)
        ...
        [usnr_map, b1_map, loss_map, rf_rate] = compute_sos_snr_mag_all(B1m, LOSS, usnr_mask_pan, idxS);
        ...
        for k = 1:size(save_items, 1)
            ...
            outfile = fullfile(FolderPath, subfolder, sprintf('dyn_%s_%s_%d.mat', prefix, flag, angle));
        ...
        for k = 1:size(correction_items, 1)
            ...
            outfile = fullfile(FolderPath, subfolder, sprintf('dyn_%s_%s_%d_correction.mat', prefix, flag, angle));
            save(outfile, 'value');
        end
