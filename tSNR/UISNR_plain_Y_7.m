...
for i = 1:length(angle_matrix)
    ...
    p = 117;
        ...
        % gpuInfo = gpuDevice();
        % memAvailable = gpuInfo.AvailableMemory; % 单位 byte
        % estimatedPerVoxel = 8e3; % 每个 voxel 大概需要的显存字节数（经验估算）
        % batchSize = floor(memAvailable / estimatedPerVoxel);
        % batchSize = min(batchSize, 1024); % 加个限制防止太大
        % [usnr_map, b1_map, loss_map, rf_rate, rf_all] = compute_usnr_gpu_batch(B1m, LOSS, usnr_mask_pan, idxS, batchSize);
        ...
        for k = 1:size(save_items, 1)
            ...
            outfile = fullfile(FolderPath, subfolder, sprintf('dyn_%s_%s_%d.mat', prefix, flag, angle_matrix(i)));
        ...
        for k = 1:size(correction_items, 1)
            ...
            outfile = fullfile(FolderPath, subfolder, sprintf('dyn_%s_%s_%d_correction.mat', prefix, flag, angle_matrix(i)));
