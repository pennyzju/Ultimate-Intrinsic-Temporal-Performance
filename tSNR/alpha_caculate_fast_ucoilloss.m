function alpha_caculate_fast_ucoilloss(B0, folderpath, flag, panmax, key_word, exclude_keyword)
    %考虑趋肤效应增加coilloss
    tic;
    ...
    biasStruct = load('/data2/jiaxinli/pan/20240801_UISNR_output/20250702dyn/loss_plain/loss_rot_0_correction.mat');  % 替换为你的真实路径
    ...
    % 保存结果
    save(fullfile(folderpath, sprintf('%s_mean_ucoilloss.mat', flag)), 'mean_map');
    save(fullfile(folderpath, sprintf('%s_std_ucoilloss.mat', flag)),  'std_map');
    save(fullfile(folderpath, sprintf('%s_cv_ucoilloss.mat', flag)),   'cv_map');
    save(fullfile(folderpath, sprintf('%s_mask_ucoilloss.mat', flag)), 'mask');
    ...
    % 导出原始图像（带 mask 轮廓）
    exportgraphics(gcf, fullfile(folderpath, sprintf('%s_raw_with_mask_ucoilloss.png', flag)), 'Resolution', 300);
    ...
    sgtitle([title_prefix ', flag = ' flag ', adducoilloss , slice = ' num2str(slice_idx)], 'Interpreter', 'none');
    % 导出统计图
    exportgraphics(gcf, fullfile(folderpath, sprintf('%s_stats_visualization_ucoilloss.png', flag)), 'Resolution', 300);
