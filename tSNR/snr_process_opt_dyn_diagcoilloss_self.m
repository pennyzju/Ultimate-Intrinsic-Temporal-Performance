function snr_process_opt_dyn_diagcoilloss_self(B0,subdir, flag, angle_matrix)
    % folderpath: 输出根目录
    ...
    coilpath = fullfile(FolderPath, [flag '_0'],BASIS_LOSS_N.mat);
    coilloss = load(coilpath);
    coilloss = real(diag(diag(coilloss.LOSS)));
    ...
    for i = 1:length(angle_matrix)
        ...
        %coilloss = 1.9117e-16;
        coilLOSS = LOSS + coilloss.*eye(2500);
    
        [usnr_map, b1_map, loss_map, rf_rate, rf_map] = compute_usnr_unacc_fast_v5(B1m, coilLOSS, usnr_mask_rot, idxS);
