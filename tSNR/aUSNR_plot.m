function aUSNR_plot(B0,folderpath, flag, panmax, key_word, exclude_keyword)
    ...
    for i = 1:N
        ...
        caxis([0, vmax]); colormap('jet');colorbar;
        title(file_names{i}, 'Interpreter', 'none');
        exportgraphics(fig, fullfile(folderpath, sprintf('%s_raw_slice_%d.png', flag, i)), 'Resolution', 300);
        close(fig);
    end
    fprintf('Raw data visualization (separate files) completed.\n');
    ...
    exportgraphics(fig, fullfile(folderpath, sprintf('%s_mean.png', flag)), 'Resolution', 300);
    close(fig);

    % 标准差图
    fig = figure('Visible','off'); colormap('jet');
    imagesc(abs(get_slice(std_map))); axis image off; title('Std'); colorbar;
    exportgraphics(fig, fullfile(folderpath, sprintf('%s_std.png', flag)), 'Resolution', 300);
    close(fig);

    % USNR图
    fig = figure('Visible','off'); colormap('jet');
    imagesc(abs(get_slice(cv_map))); axis image off; title('USNR'); colorbar;
    exportgraphics(fig, fullfile(folderpath, sprintf('%s_usnr.png', flag)), 'Resolution', 300);
    close(fig);
    ...
    fprintf('Statistical visualization (separate files) completed. Total time elapsed: %.2f seconds\n', toc);
end
