...
for i = 1:length(angle_matrix)
    ...
    elseif strcmp(flag, 'rot')
        ...
        % 与原始矩阵求交集
        usnr_mask = usnr_mask.*rotated_mask;       

        LOSS=LOSS(1:size(B1m,2),1:size(B1m,2));

        [usnr_map,b1_map,loss_map]=compute_usnr_unacc_fast_v4(B1m,LOSS,usnr_mask,idxS);
        ...
        save(Xout_file3, 'usnr_mask');  % 默认保存格式
        ...
        Xout_file5 = fullfile(FolderPath, subfolders{1}, sprintf('loss_%s_%d_correction.mat', flag, angle_matrix(i)));
        save(Xout_file4, 'loss_map_correction');
        clear usnr_map b1_map loss_map B1m loss_map_anti usnr_map_anti
