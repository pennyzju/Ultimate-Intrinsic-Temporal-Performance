function lamda_caculate_fast(B0,folderpath, flag, panmax,seq, key_word, exclude_keyword)
    tic;
    panmatrix = -panmax:1:panmax;
    file_names = {};
    data_list = [];
    addpath(genpath('/data/jiaxinli/projects/20240728_my-repo_test'));
    folderpath = fullfile(B0_folderpath(B0),folderpath,'B1_map');
    ...
    title_prefix = ['lamda_' folder_tag '_seq' num2str(seq)];
    ...
    for i = 1:length(panmatrix)
        ...
        % 进一步要求包含 seq
        matched_flags = matched_flags & contains({all_files.name}, seq_str);
    ...
    % 保存结果
    save(fullfile(folderpath, sprintf('%s_mean_seq%d.mat', flag, seq)), 'mean_map');
    save(fullfile(folderpath, sprintf('%s_std_seq%d.mat', flag, seq)),  'std_map');
    save(fullfile(folderpath, sprintf('%s_cv_seq%d.mat', flag, seq)),   'cv_map');
    save(fullfile(folderpath, sprintf('%s_mask_seq%d.mat', flag, seq)), 'mask');
    ...
    sgtitle([title_prefix ', flag = ' flag ', seq = ' num2str(seq) ',slice = ' num2str(slice_idx)], 'Interpreter', 'none');
    % 导出原始图像（带 mask 轮廓）
    exportgraphics(gcf, fullfile(folderpath, sprintf('%s_raw_with_mask_seq%d.png', flag, seq)), 'Resolution', 300);
    ...
    subplot(2,2,3); imagesc(abs(get_slice(cv_map)));   axis image off; title('lamda'); colorbar;
    subplot(2,2,4); imagesc(get_slice(mask));          axis image off; title('Mask'); colorbar;
    sgtitle([title_prefix ', flag = ' flag ', seq = ' num2str(seq) ' slice = ' num2str(slice_idx)], 'Interpreter', 'none');
    % 导出统计图
    exportgraphics(gcf, fullfile(folderpath, sprintf('%s_stats_visualization_seq%d.png', flag, seq)), 'Resolution', 300);
