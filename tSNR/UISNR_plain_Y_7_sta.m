...
angle_matrix(angle_matrix == 0) = [];  % 去除0度
...
for i = 1:length(angle_matrix)
    ...
    for k = 1:size(save_items,1)
        ...
        save(fullfile(FolderPath, folder, sprintf('sta_%s_%s_%d.mat', prefix, flag, angle)), v);
    end
    ...
    for k = 1:size(corr_items,1)
        ...
        save(fullfile(FolderPath, folder, sprintf('sta_%s_%s_%d_correction.mat', prefix, flag, angle)), 'shifted');
    end
