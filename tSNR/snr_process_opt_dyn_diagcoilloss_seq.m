function snr_process_opt_dyn_diagcoilloss_seq(B0,subdir, flag, angle_matrix,seqlist)
    % folderpath: 输出根目录
    ...
    for i = 1:length(angle_matrix)
        ...
        coilloss = diag(diag(coilloss.LOSS)); 
        totLOSS = LOSS + 100*coilloss;
        
        for seq = seqlist
            B1mseq = B1m(:,1:seq);
            totLOSSseq = totLOSS(1:seq,1:seq);
            [usnr_map, b1_map, loss_map, rf_rate, rf_map] = compute_usnr_unacc_fast_v5(B1mseq, totLOSSseq, usnr_mask_rot, idxS);
            ...
            save(fullfile(FolderPath, subdir, subfolders{3}, sprintf('mask_%s_%d_%d.mat', flag, angle, seq)), 'usnr_mask_rot');
