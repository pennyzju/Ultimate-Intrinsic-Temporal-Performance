function snr_process_sos_mag(FolderPath, flag, angle_deg, subdir)
    % FolderPath: 输出根目录
    % flag: 'X'、'Y'、'Z' 或 'front'、'left'、'rot'
    % angle_deg: 最大角度（整数），会从 -angle_deg 到 angle_deg 处理
    % subdir: 保存结果的子文件夹（如 'sos_mag'）
    ...
    for i = 1:length(angle_matrix)
        ...
        [usnr_map, b1_map, loss_map, rf_rate] = compute_sos_snr_mag(B1m, LOSS, usnr_mask_pan, idxS);
