function snr_process_sos(FolderPath, flag, angle_deg, subdir,abs_rf, abs_q)
    ...
    for angle = angle_matrix
        ...
        if ~exist(subFolderPath, 'dir')
            fprintf('文件夹不存在，跳过: %s\n', subFolderPath); continue;
        end

        cd(subFolderPath);
        try
            load('BASIS_B1m'); load('object_def'); load('BASIS_LOSS_N');
        catch ME
            fprintf('加载基础文件时出错: %s\n', ME.message); continue;
        end

        matFiles = dir('*material*.mat');
        if isempty(matFiles), fprintf('未找到 material 文件。\n'); continue; end
        try
            load(matFiles(1).name);  % pan_mask
        catch ME
            fprintf('加载文件 %s 出错: %s\n', matFiles(1).name, ME.message); continue;
        end
