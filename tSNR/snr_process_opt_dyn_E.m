function snr_process_opt_dyn_E(B0,subdir, flag, angle_matrix)
    % folderpath: 输出根目录
    ...
    for i = 1:length(angle_matrix)
        ...
        LOSS = LOSS(1:2:size(LOSS,1)-1,1:2:size(LOSS,1)-1);
        B1m = B1m(:,1:2:size(B1m,2)-1);
    
        [usnr_map, b1_map, loss_map, rf_rate, rf_map] = compute_usnr_unacc_fast_v5(B1m, LOSS, usnr_mask_rot, idxS);
