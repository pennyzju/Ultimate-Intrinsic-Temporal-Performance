function snr_process_sos_dyn_old(FolderPath, flag, angle_deg)
    % folderpath: 输出根目录
    ...
    for i = 1:length(subfolders)
        ...
        disp(['Checking folder: ', folder_name]);

try
    dirinfo = dir(folder_name);
    disp('dir() succeeded.');
catch ME
    warning('dir() failed: %s', ME.message);
end
...
if ~exist(folder_name, 'dir')
    try
        ...
        warning('mkdir failed: %s', ME.message);
    end
else
    disp('Folder exists.');
end
