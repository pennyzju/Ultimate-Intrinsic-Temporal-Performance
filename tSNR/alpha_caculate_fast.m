function alpha_caculate_fast(B0, folderpath, flag, panmax, key_word, exclude_keyword)
    tic;
    ...
    % 保存结果
    save(fullfile(folderpath, sprintf('alpha_%s_mean.mat', flag)), 'mean_map');
    save(fullfile(folderpath, sprintf('alpha_%s_std.mat', flag)),  'std_map');
    save(fullfile(folderpath, sprintf('alpha_%s_cv.mat', flag)),   'cv_map');
    save(fullfile(folderpath, sprintf('alpha_%s_mask.mat', flag)), 'mask');
    ...
    % 导出原始图像（带 mask 轮廓）
    exportgraphics(gcf, fullfile(folderpath, sprintf('alpha_%s_raw_with_mask.png', flag)), 'Resolution', 300);
    ...
    sgtitle([title_prefix ', flag = ' flag ', addcoilloss , slice = ' num2str(slice_idx)], 'Interpreter', 'none');
    % 导出统计图
    exportgraphics(gcf, fullfile(folderpath, sprintf('alpha_%s_stats_visualization.png', flag)), 'Resolution', 300);
