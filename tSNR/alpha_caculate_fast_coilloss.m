function alpha_caculate_fast_coilloss(B0, folderpath, flag, panmax, key_word, exclude_keyword)
    tic;
    ...
    folderpath = fullfile(B0_folderpath(B0),folderpath,'loss_plain');
    ...
    %title_prefix = ['alpha_' folder_tag];
    title_prefix = ['alpha_' folder_tag 'addcoilloss'];


    fprintf('Reading data files...\n');
    % Collect files matching the criteria
    biasStruct = load('/data2/jiaxinli/pan/20240801_UISNR_output/20250702dyn/loss_plain/loss_left_0_correction.mat');  % 替换为你的真实路径
    bias = biasStruct.loss_map_correction;  % 假设变量名为 bias
    ...
    % 保存结果
    save(fullfile(folderpath, sprintf('%s_mean_coilloss.mat', flag)), 'mean_map');
    save(fullfile(folderpath, sprintf('%s_std_coilloss.mat', flag)),  'std_map');
    save(fullfile(folderpath, sprintf('%s_cv_coilloss.mat', flag)),   'cv_map');
    save(fullfile(folderpath, sprintf('%s_mask_coilloss.mat', flag)), 'mask');
    ...
    sgtitle([title_prefix ', flag = ' flag ', addcoilloss ,slice = ' num2str(slice_idx)], 'Interpreter', 'none');
    % 导出原始图像（带 mask 轮廓）
    exportgraphics(gcf, fullfile(folderpath, sprintf('%s_raw_with_mask_coilloss.png', flag)), 'Resolution', 300);
    ...
    sgtitle([title_prefix ', flag = ' flag ', addcoilloss , slice = ' num2str(slice_idx)], 'Interpreter', 'none');
    % 导出统计图
    exportgraphics(gcf, fullfile(folderpath, sprintf('%s_stats_visualization_coilloss.png', flag)), 'Resolution', 300);
