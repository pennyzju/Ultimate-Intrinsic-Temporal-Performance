...
function ubasis_solve(public_out_dir,out_dir,fileName)
...
xd = r(:,:,:,1);
xmin = min(xd(idxS_big))-dx; xmax = max(xd(idxS_big))+dx;
yd = r(:,:,:,2);
ymin = min(yd(idxS_big))-dx; ymax = max(yd(idxS_big))+dx;
zd = r(:,:,:,3);
zmin = min(zd(idxS_big))-dx; zmax = max(zd(idxS_big))+dx;
...
mask_new = mask_big(idxX,idxY,idxZ);
...
% Load the basis
Out_file = sprintf('%s/BASIS_Einc.mat', public_out_dir);
load(Out_file);
...
%}
    ...
    if 1  % ((ii/100) == floor(ii/100))
        ...
        % 显示最后一次写入的日志内容
        fprintf('\n %d Solves: Elapsed time = %f   flag = %3.0f   iter = %3.1f   relres = %e\n', ii, etime(t2,t1), flag, iter, relres);
    end
    
    % SAVE JSOL EVERY 100 FIELDS IN CASE O F SIMULATION CRASH
    if mod(ii,100) == 0 || ii==ncol
        
        TEMP = Jsol( :,INDEX_SAVE_PRE :ii );
        %eval( sprintf('save -v7.3 Jsol_TEMP_%dTO%d.mat TEMP;',INDEX_SAVE_PRE,ii) );
        save( sprintf('%s/Jsol_TEMP_%dTO%d.mat', out_dir, INDEX_SAVE_PRE, ii), 'TEMP', '-v7.3');
