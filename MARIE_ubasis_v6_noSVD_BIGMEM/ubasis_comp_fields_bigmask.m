function ubasis_comp_fields_bigmask(public_out_dir,out_dir,fileName)
    ubasis_options = load(fullfile(public_out_dir, 'ubasis_options.mat'));
    
    object_def = load(fullfile(out_dir, 'object_def_bigmask.mat'));
    load(fullfile(out_dir, fileName));%导入材料数据
    
    % load basis solution
    BASIS_Jsol =load(fullfile(out_dir, 'bigmask','BASIS_Jsol.mat')); 
    ...
    % Define the output file path using sprintf
    Out_file = sprintf('%s/bigmask/BASIS_Etot.mat', out_dir);

    % Save the variable 'Ebasis' in the specified path with the '-v7.3' option
    ...
    SigN = rotated_sigma_e(object_def.idxN);
    RhoN = rotated_rho(object_def.idxN);
    ...
    Xout_file = sprintf('%s/bigmask/BASIS_LOSS.mat', out_dir);
    save(Xout_file, 'LOSS', '-v7.3');
    Xout_file = sprintf('%s/bigmask/BASIS_GSAR.mat', out_dir);
    ...
    Xout_file = sprintf('%s/bigmask/BASIS_LOSS_N.mat', out_dir);
    save(Xout_file, 'LOSS', '-v7.3');
    Xout_file = sprintf('%s/bigmask/BASIS_GSAR_N.mat', out_dir);
    save(Xout_file, 'GSAR', '-v7.3');
    ...
    Xout_file = sprintf('%s/bigmask/BASIS_B1p.mat', out_dir);
    save(Xout_file, 'B1p', '-v7.3');
    
    B1m = ( squeeze(Hbasis(:,1,:)) - 1j*squeeze(Hbasis(:,2,:)) ) / sqrt(2);
    Xout_file = sprintf('%s/bigmask/BASIS_B1m.mat', out_dir);
