function [Eout,Jout,fG] = jvie_solver(Vin, e_r, r, res, f, fG, tol)
...
% non-air media
fA   = @(J,sTrans)mv_op_fft_Nop_2(J, fG, e_r, dV, sTrans, idxS);
...
[vSol,flag,~,~,resvec] = pgmresDR(@(J)fA(J,'notransp'), Vexc(idxS), maxit, tol, maxout);


Time= toc(tic_drgmres);
residue =  Vexc(idxS) - fA(vSol,'notransp');
fprintf(1,' Solve Done:  Elapsed time  = %g [sec], flag %d, %d iterations, residue %f, relative %g \n' , Time,flag,length(resvec),norm(residue),norm(residue)/norm(Vexc(idxS)));
