...
if (FORMULATION == 1) % JVIE_I formulation
    ...
    if (GPU && gpuDeviceCount && (METHOD~=3) && (METHOD~=1) && (METHOD~=11)) % USE GPU ADAPTED FUNCTIONS
        ...
        fA   = @(J,transp_flag)mv_op_fft_1_gpu(J, FG_gpu, EPS_gpu, dV, transp_flag, idxS);
        % define the preconditioner
        fP   = @(J,transp_flag)mv_op_fft_1_gpu(J, FG_gpu, 1./EPS_gpu, dV, transp_flag, idxS);
    
    else
        
        % define the main function
        fA   = @(J,transp_flag)mv_op_fft_1(J, FG, EPS, dV, transp_flag, idxS);
        % define the preconditioner
        fP   = @(J,transp_flag)mv_op_fft_1(J, FG, 1./EPS, dV, transp_flag, idxS);
    ...
    if (GPU && gpuDeviceCount && (METHOD~=3) && (METHOD~=1) && (METHOD~=11)) % USE GPU ADAPTED FUNCTIONS
        ...
        % define the preconditioner
        fP   = @(J,transp_flag)mv_op_fft_2_gpu(J, FG_gpu, 1./EPS_gpu, dV, transp_flag, idxS);
        
    else
        
        % define the main function
        fA   = @(J,transp_flag)mv_op_fft_2(J, FG, EPS, dV, transp_flag, idxS);
        % define the preconditioner
        fP   = @(J,transp_flag)mv_op_fft_2(J, FG, 1./EPS, dV, transp_flag, idxS);
