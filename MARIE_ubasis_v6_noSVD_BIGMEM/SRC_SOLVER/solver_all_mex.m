function [Jout,Sout,Eout,Bout,fG,fK,resvec,titer] = solver_all_mex(Ein, Hin, f, r, Eps_r, Sig_e, Rho, out, tol, bicgtrue, nogpu)
...
if bicgtrue
    ...
    if (bicgtrue == 1)
        ...
        tau = zeros(LfG,MfG,NfG);
        tau(1:L,1:M,1:N) = (e_rnew - 1 )./ e_rnew;
        FIDX = find(tau(:));
        DIM = length(FIDX);
        TAU = tau(FIDX);
        
        % [vSol,flag,resvec] = mexFFTW_JVIEII_Solver(fG(:), Vexc(:), TAU(:), FIDX(:), dV, DIM, LfG, MfG, NfG, tol, maxit);
        [vSol,flag,resvec] = ompFFTW_JVIEII_Solver(fG(:), Vexc(:), TAU(:), FIDX(:), dV, DIM, LfG, MfG, NfG, tol, maxit);
        
        
        Time= toc(tic_solve);
        residue =  Vexc - fA(vSol,'notransp');
        fprintf(1,'\n Current Distribution Computed:\n MEX BICGSTAB Elapsed time  = %g [sec], flag %d, %d iterations, residue %f, relative %g \n' , Time,flag,length(resvec),norm(residue),norm(residue)/norm(Vexc));
