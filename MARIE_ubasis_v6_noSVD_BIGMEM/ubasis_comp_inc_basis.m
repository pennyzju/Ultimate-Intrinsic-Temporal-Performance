...
function ubasis_comp_inc_basis(public_out_dir)
...
filename = sprintf('%s/object_def.mat',public_out_dir);
load(filename);
...
% e_r = epsilon_r - 1j*sigma_e/(eo*omega);%存疑，不知道什么意思
...
Out_file = sprintf('%s/BASIS_Hinc.mat', public_out_dir);
