function ubasis_comp_fields(public_out_dir,out_dir,fileName)
...
idxS3_big = [object_def.idxS_big; nD+object_def.idxS_big; 2*nD+object_def.idxS_big];% the vector of all mask positions in 3D grid
% 使用 ismember 查找 idxS3 在 idxS3_big 中的位置
[isMember, locations] = ismember(idxS3, idxS3_big);
...
for ii = 1:Nexc
    ...
    ve(idxS3) = BASIS_Einc.Ebasis(locations,ii); % transfer to global vars
    vj(idxS3) = BASIS_Jsol.Jsol(locations,ii); % transfer to global vars
...
for ii = 1:Nexc
    ...
    vm(idxS3) = BASIS_Hinc.Hbasis(locations,ii); % transfer to global vars
    vj(idxS3) = BASIS_Jsol.Jsol(locations,ii); % transfer to global vars
